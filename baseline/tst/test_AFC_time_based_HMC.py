...
if not SKIP_TEST:
    ...
    _FILENAME = "240626_AFC_r5.2.1_e-pit_addChgStatus.csv"
    ...
    def parse_AFC_Behavioral_Test_20240423():
        ...
        with open(csv_path, "r") as file:
            ...
            for row in reader:
                # Get inputs
                test_filename = '240626_AFC_r5.2.1_e-pit'
                Time = int(row["timestamp"])
                PackSOC = int(row["Real pack SOC"])
                PackSOC_DR = 1
                PackCurr = -int(row["Input_PackCurr"])
                PackCurr_DR = 1

                try:
                    # CellVolts = [int(item) for item in ast.literal_eval(row["CellVolts"])]
                    CellVolts = [int(row[f'Input_CellVolts_{i}']) for i in range(1, 193)]
                except TypeError:
                ...
                try:
                    ...
                    TempSnsrs = [int(row[f'Input_TempSnsrs_{i}']) for i in range(0, 18)]
                except TypeError:
                ...
                MinTempSnsr = int(row["Input_MinTempSnsr"])
                MinTempSnsr_DR = 1

                MaxTempSnsr = int(row["Input_MaxTempSnsr"])
                MaxTempSnsr_DR = 1

                ChgPackCapcty = 120000
                ChgPackCapcty_DR = 1
    ...
    def test_AFC_Behavioral_Test_20240423(lib):
        ...
        for each_time_step in all_time_steps:
            ...
            results["AFC_SW_Version"].append(lib.get_afc_sw_ver())
            results["Config_Version"].append(lib.get_afc_cfg_ver())
            ...
            results["QnovoAFC_LogVar7\nl_Highest_Index"].append(lib.QnovoAFC_LogVar7)
