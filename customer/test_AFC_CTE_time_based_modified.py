...
if not SKIP_TEST:
    ...
    LOG_FILE = "cte_test_log_30_45_direct_call.txt"

    module_path = abspath(__file__)
    _OUTPUT = "test_data/time_based/output_data"
    ...
    def test_hmc_afc_cte_behavioural(lib):
        ...
        for each_time_step in all_time_steps:
            ...
            if charging_started:
                ...
                if battery_state == 1 and lib.cte_estimates.charging.time_end_soc == 0 and charging_100_completed is False:
                    ...
                    if expected_charge_time_100 and actual_charge_time_100 and hmc_time_end_soc:
                        ...
                        write_to_log(f"Deviation in estimate for 100% capacity current charger HMC actual/Test Actual: {deviation_100}")
                        if deviation_100 > ACCEPTED_CHARGE_TIME_DEVIATION_PERCENTAGE:
                            write_to_log(f"Deviation in estimate for current charger 80% capacity is above KPI: {deviation_100}")
                    ...
                    if hmc_start_estimate_for_end_soc and expected_charge_time_100:
                        ...
                        if deviation_80 > ACCEPTED_CHARGE_TIME_DEVIATION_PERCENTAGE:
                            write_to_log(f"Error: END SOC Deviation in estimate HMC estimate and Test Estimate above KPI: {deviation_100}")
                    ...
                    if expected_charge_time_100 and actual_charge_time_100:
                        ...
                        deviation_100 = (diff / average) * 100
                        write_to_log(f"END SOC Deviation in estimate Test estimate and Test Actual above KPI: {deviation_100}")
                        if deviation_100 > ACCEPTED_CHARGE_TIME_DEVIATION_PERCENTAGE:
                            write_to_log(f"Error: END SOC Deviation in estimate Test estimate and Test Actual above KPI: {deviation_100}\n")
                ...
                if battery_state == 1 and lib.cte_estimates.charging.time_80 == 0 and charging_80_completed is False:
                    ...
                    if expected_charge_time_80 and actual_charge_time_80 and hmc_time_80:
                        write_to_log(f"80% capacity: HMC actual charge time: {hmc_time_80} and Test actual charge time: {actual_charge_time_80}")
                        ...
                        write_to_log(f"Deviation in estimate for 80% capacity current charger HMC actual/Test Actual: {deviation_80}")
                        if deviation_80 > ACCEPTED_CHARGE_TIME_DEVIATION_PERCENTAGE:
                    ...
                    if hmc_start_estimate_for_80 and expected_charge_time_80:
                        ...
                        if deviation_80 > ACCEPTED_CHARGE_TIME_DEVIATION_PERCENTAGE:
                            write_to_log(f"Error: Deviation in estimate HMC estimate and Test Estimate above KPI: {deviation_80}")
                    ...
                    if expected_charge_time_80 and actual_charge_time_80:
                        ...
                        if deviation_80 > ACCEPTED_CHARGE_TIME_DEVIATION_PERCENTAGE:
                            write_to_log(f"Error: Deviation in estimate Test estimate and Test Actual above KPI: {deviation_80}\n")
            ...
            if battery_state == 0 and charging_started is True:
                ...
                write_to_log(f"This is an estimation based on current END SOC with Test (Which may not be 100% charging recahed")
                soc_end_time_100 = input_time
                charging_100_completed = True
                actual_charge_time_100 = soc_end_time_100 - soc_start_time_100
                write_to_log(
        ...
        dir_path = join(dirname(module_path), _OUTPUT)
        csv_path = join(dir_path, f"processed_direct_CTE_{_FILENAME}")
