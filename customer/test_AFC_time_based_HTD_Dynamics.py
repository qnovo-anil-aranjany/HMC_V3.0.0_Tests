"""Test Module Description:
    Test module for HSD time-based testing. This test validates HTD and Slew rate functionality.
...
if not SKIP_TEST:
    ...
    _OUTPUT_DIR = _BASE_DIR / "output_data"
    _INPUT_DIR = _BASE_DIR / "input_data"
    _REFERENCE_DIR = _BASE_DIR / "reference_data"
    _FILENAME = "TimeBased_20260205_HTD_Dynamics.csv"

    def parse_AFC_Behavioral_Test_20240423():
        module_path = abspath(__file__)
        dir_path = join(dirname(module_path), _INPUT_DIR)
    ...
    def test_AFC_Behavioral_Test_20240423(lib):
        ...
        output_file_name = join(dirname(_BASE_DIR), _OUTPUT_DIR, f"processed_{_FILENAME}")
        write_output_to_excel(results, output_file_name)
        output_pack_curr = results["ChgPackCurr"]
        reference_file = join(dirname(_BASE_DIR), _REFERENCE_DIR, f"reference_{_FILENAME}")
