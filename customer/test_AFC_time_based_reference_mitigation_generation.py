...
if not SKIP_TEST:
    ...
    _FILENAME = "InputDataForMitigation.csv"
    ...
    def test_AFC_Behavioral_Test_20240423(lib):
        ...
        for each_time_step in all_time_steps:
            ...
            if lib.Output_SlicingStatus != 0:
                ...
                continue
            if slice_count != 10:
                pytest.fail(f"Slice count is : {slice_count}")
                break
            # Record results
            ...
            lib.LIB_Deobfuscate(ffi.addressof(lib.s_AFC_CTE_Data, "VaAFC_Cnt_CPVCorrIdxCte"), size(lib.s_AFC_CTE_Data.VaAFC_Cnt_CPVCorrIdxCte), 0xBD)
            results["AFC_CTE_HighestIndex"].append(lib_array_to_list(lib.s_AFC_CTE_Data.VaAFC_Cnt_CPVCorrIdxCte))
        ...
        csv_path = join(output_path, f"processed_{_FILENAME}")
        test_output_curr = results["ChgPackCurr"]
        test_output_curr_1 = [int(i) for i in test_output_curr]
        ...
        csv_path = join(reference_path, f"reference_{_FILENAME}")
        # Open the CSV file
        with open(csv_path, "r") as file:
            reader = csv.DictReader(file)
            reference_output_curr = []
            for row in reader:
                reference_output_curr.append(int(row["ChgPackCurr"]))
        compare_result(test_output_curr_1, reference_output_curr)
