"""Test Module Description:
    Test module for checking Extreme, Abnormal and Early warnings are being generated based on mitigations.
    Test data provide input cell volts that trigger mitigations for charge cycles that match with
    expected range for each warning flags.
...
MAKE_HTML = True
if not SKIP_TEST:
    _FILENAME = "Behavioral_Test_logging_all_warnings.xlsx"
    ...
    def parse_AFC_logging_test_data(file_path):
        ...
        for row in iter_file(file_path):
            try:
                ...
                try:
                    CellVolts = [
                        int(float(item)) for item in ast.literal_eval(row["CellVolts"])
                    ]
                except TypeError:
    ...
    def test_AFC_logging_behavioral(lib: Any, setup_parameters):
        ...
        for i in range(1):
            ...
            print(f"Error Flags : {lib.VeAFC_e_ErrorFlags}")
            print(f"SOCImbalanceFlag: {lib.VeAFC_b_SOCImbalanceFlag}")
            print(f"Charge Completion Flag: {lib.VeAFC_b_ChgCompletionFlag}")
            print(f"Charge Pack Volt: {lib.VeAFC_U_ChgPackVolt}")
            print(f"Mitigated Current: {lib.VeAFC_I_MitigatedCurr}")
            print(f"Max Reference Current: {lib.VeAFC_I_MaxReferenceCurr}")
            print(f"Charge Pack Current: {lib.VeAFC_I_ChgPackCurr}\n")
            ...
            for k in range(1):
                ...
                for each_time_step in all_time_steps:
                    ...
                    # NVM load with power cycle
                    if row_count == 918:
                        print(f"\nAFC Init: Power cycle simulate: {lib.s_AFC_Track.Ne_Cnt_ChargeCycleNum}")
                        lib.Qnovo_AFC_Init()
                    ...
                    if not charging_started and lib.VeAPI_b_EVSEChgStatus == 1:
                        charging_started = 1
                    slice_count = 1
                    while True:
                        # Run Function
                        ...
                        if lib.Output_SlicingStatus == 0:
                            if slice_count != 10:
                                print("\nError: AFC slicing status 0 before 10 cycles")
                            break
                        slice_count += 1

                    if lib.Output_SlicingStatus != 0:
                        print("\nError: With 10 slices, we did not get Slicing status 0")
                        continue
                    if charging_started and lib.VeAPI_b_EVSEChgStatus == 0:
                        charging_started = 0
            ...
            print(f"Extreme Flags: {lib.VeAFC_b_ExtremeAgingFlag}")
            print(f"EOL Flags: {lib.s_AFC_Track.Ne_b_EOLFlag}")
            compare_result(1, lib.VeAFC_b_EarlyWarningAgingFlag)
            compare_result(1, lib.VeAFC_b_AbnormalAgingFlag)
            compare_result(1, lib.VeAFC_b_ExtremeAgingFlag)
